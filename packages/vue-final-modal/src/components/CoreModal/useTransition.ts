...
function useTransitionState(
  _visible = false
): [Ref<boolean>, Ref<undefined | TransitionState>, TransitionListeners] {
  const visible = ref(_visible);
  const state = ref<undefined | TransitionState>(
    visible.value ? TransitionState.Enter : undefined
  );
...
export function useTransition(
  ...
  const contentTransition = computed<TransitionProps>(() => {
    ...
    return Object.assign({}, props.contentTransition);
  });
  ...
  const overlayTransition = computed<TransitionProps>(() => {
    ...
    return Object.assign({}, props.overlayTransition);
  });
  ...
  watch(isReadyToBeDestroyed, (value) => {
