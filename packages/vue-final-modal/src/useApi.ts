...
function withMarkRaw<P>(
  options: Partial<UseModalOptions<P>>,
  DefaultComponent: Component = VueFinalModal
) {
 ...
 */
export function useModal<P = InstanceType<typeof VueFinalModal>["$props"]>(
  _options: UseModalOptions<P>
): UseModalReturnType<P> {
  ...
  function patchComponentOptions<P>(
    ...
    if (newOptions.component) options.component = newOptions.component;

    if (newOptions.attrs) patchAttrs(options.attrs!, newOptions.attrs);
  }

  function patchAttrs<T extends Record<string, any>>(
    attrs: T,
    newAttrs: Partial<T>
  ): T {
  ...
  function destroy(): void {
    ...
    if (index !== -1) vfm.dynamicModals.splice(index, 1);
  }
...
export function byPassAllModalEvents(
...
export function useVfmAttrs(options: {
  ...
  const vfmAttrs = computed(() =>
    Object.assign(bindProps.value, bindEmits, attrs)
  );
